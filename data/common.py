...
def generate_system_prompt(system_prompt_type="caption", vision_type="video"):
    ...
    if system_prompt_type == "caption":
    ...
    elif "idip" in system_prompt_type or system_prompt_type == "tiv2v":
        ...
        str_list = [f"Describe the key features of the input media (color, shape, size, texture, objects, background, style), then incorporate the user's text description to generate a new {vision_type} that satisfies the user's requirements while drawing visual inspiration from the reference inputs."]
